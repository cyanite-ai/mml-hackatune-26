...
- **Find by text prompt**: `POST /library-tracks/prompt-search`, body `{"query": "..."}`
- **Find similar (single seed)**: `POST /library-tracks/{id}/similar-tracks`
- **Find similar (multi-track, up to 10 seeds)**: `POST /library-tracks/similarity-search`, body `{"tracks": [{"id": "libtr_..."}]}`
