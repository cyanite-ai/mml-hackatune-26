...
1. **Search** returns a ranked list of results, each `{track, score}` where `score` is a
   ...
   - **Find by text prompt**: natural language to matching tracks (`POST /library-tracks/prompt-search`).
   - **Find similar (single seed)**: a track to acoustically similar tracks (`POST /library-tracks/{id}/similar-tracks`).
   - **Find similar (multi-track, up to 10 seeds)**: similar to the mean of several tracks, e.g. a user's likes (`POST /library-tracks/similarity-search`).
